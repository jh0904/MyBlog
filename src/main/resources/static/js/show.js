    ...
    function putInArticle(data) {
        ...
        var articleTop = $('<article-top><div class="article-title">' +
            ...
            '版权声明：本文版权归   <b>'+data.originalAuthor+'</b>  所有，欢迎转载，但未经作者同意必须保留原文链接。' +
            '</div>');
        ...
        var articleFooter = $('<div class="end-logo">' +
            ...
            '</span>' +
            '</div>');
        $('.blog-article-footer').append(articleFooter);
        ...
        $('.other').append($('<div class="social-share" data-initialized="true" data-url="https://www.baidu.cn/findArticle?articleId=' + data.articleId + '&originalAuthor='+ data.originalAuthor + '"  data-title="' + data.articleTitle + '">' +
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.status == "200"){
                ...
                $('.content').html('');
                var error = $('<div class="article"><div class="blog-article-top"><div class="error">' +
                    '<img src="https://blog-myblog.oss-cn-shenzhen.aliyuncs.com/static/img/register_success.jpg">' +
                    '<p>没有找到这篇文章哦</p>' +
                    '<p>可能不小心被博主手残删掉了吧</p>' +
                    '<div class="row">' +
                    '<a href="/">返回首页</a>' +
